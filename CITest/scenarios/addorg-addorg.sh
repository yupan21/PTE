...
function restartChaincode(){
    ...
    chaincode_name=$1
    cd $CISCRIPT_DIR
    node ./config.js $testcasename$testcaseconfigfile1 chaincodeID $chaincode_name
    node ./config.js $testcasename$testcaseconfigfile2 chaincodeID $chaincode_name
    node ./config.js $testcasename$testcaseconfigfile3 chaincodeID $chaincode_name
    bash test_driver.sh -m $testcasename -c samplecc
    # restart chaincode ---------------
}
restartChaincode sample_cc
